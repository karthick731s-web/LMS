...
export function TLogin() {
    ...
    const handlesubmit = async () => {
        try {
            const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/instructors/login`, { email, password });
            if (res.status === 200) {
                localStorage.setItem("token", res.data.token);
                localStorage.setItem("role", res.data.role);
                localStorage.setItem("userName", res.data.userName || "Instructor");
                ...
                <div className="w-full md:w-[40%] h-full bg-[#1a2d45] flex items-center justify-center p-6 md:p-10">
                    ...
                    <div className="grid gap-3 w-full max-w-md bg-white rounded-2xl px-6 py-8 shadow-xl">
                        <div className="text-center mb-6">
                         ...
                         <div className="text-center mb-6">
                            <h1 className="text-2xl font-bold text-[#1a2d45]">Instructor Login</h1>
                        </div>
                        {/* Email */}
                        <div className="mb-5">
